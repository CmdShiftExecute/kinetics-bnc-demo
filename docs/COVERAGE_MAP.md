...
Verification evidence referred to below: `bun run reconcile` (2,562 cross-table assertions across 11 categories, published on the Data basis page), `bun scripts/interactions.ts` (89 browser checks), `bun run screenshots` (every route at 1440, 1024 and 390 pixels, plus the Projects page filtered and the matrix rolled up to sector level), and `bun run stable` (byte-stability across two runs).
...
|---|---|---|
| Consultant Relationship Matrix | `parties/consultants.json`, one `Party` per consultant (kind `consultant`), with `role`, `verticals`, `level`, `rating`, `owner`, `projectCount`, `projectValue` and `projects`; browsed on `/parties` | Adapted: one party table carrying a `verticals` list per firm, in place of a matrix with a vertical down every column, because the number of verticals a firm touches is small enough to list rather than tabulate |
| Contractor Relationship Matrix | `parties/contractors.json`, the same shape with kind `contractor`, role `lead`, `mep` or `both` | Adapted, same reasoning |
